...
#' use [psyverse::dct_from_spreadsheet()].
#'
...
read_spreadsheet <- function(x,
  ...
  gSheetId_extractionRegex <- psyverse::opts$get("gSheetId_extractionRegex");
  gSheetId_to_exportLink <- psyverse::opts$get("gSheetId_to_exportLink");
  ...
  if (!is.null(sheet)) {
    ...
    if (all(sheet %in% names(res))) {
      ...
      res <- res[sheet];
      msg("Selected sheet(s) with numbers ", vecTxtQ(sheet), "!\n",
          silent = silent);
    } else {
